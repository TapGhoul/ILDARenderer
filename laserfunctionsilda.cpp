...
std::vector<coordinate_data> LaserFunctionsILDA::spinCubeYZ(int time)
{
    ...
    int sX = sin(angle)*4000;
    int sY = cos(angle)*4000;
    int scale = 1000;
    ...
    coords[9] = setPoint(-sY, sX, 2);
    coords[10] = setPoint(scale/2, scale/2, 2);
    coords[11] = setPoint(scale-sY, scale+sX);
    coords[12] = setPoint(scale-sX, scale-sY, 3);
    coords[13] = setPoint(scale/2, scale/2, 3);
    coords[14] = setPoint(-sX, -sY);
    coords[15] = setPoint(sY, -sX, 4);
    coords[16] = setPoint(scale/2, scale/2, 4);
    coords[17] = setPoint(scale+sY, scale-sX);
    coords[18] = setPoint(scale+sX, scale+sY, 5);
